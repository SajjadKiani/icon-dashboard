import * as React from "react";
import type { SVGProps } from "react";
const SvgVibeVibeOutline = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    {...props}
  >
    <path
      fill="#171717"
      d="M12 12.6c-.24 0-.46-.11-.6-.3L8.9 8.97a.75.75 0 0 1-.07-.79c.13-.26.39-.41.67-.41h5c.28 0 .54.16.67.41s.1.56-.07.79l-2.5 3.33c-.14.19-.36.3-.6.3m-1-3.33 1 1.33 1-1.33z"
    />
    <path
      fill="#171717"
      d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12S6.07 1.25 12 1.25 22.75 6.07 22.75 12 17.93 22.75 12 22.75m0-20C6.9 2.75 2.75 6.9 2.75 12S6.9 21.25 12 21.25s9.25-4.15 9.25-9.25S17.1 2.75 12 2.75"
    />
    <path
      fill="#171717"
      d="M12 17.75c-.24 0-.47-.12-.61-.31l-5-7a.76.76 0 0 1 .17-1.05.76.76 0 0 1 1.05.17L12 15.71l4.39-6.15c.24-.34.71-.42 1.05-.17.34.24.42.71.17 1.05l-5 7c-.14.19-.37.31-.61.31"
    />
  </svg>
);
export default SvgVibeVibeOutline;
